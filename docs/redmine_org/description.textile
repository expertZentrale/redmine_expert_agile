...
* *Sprints* — a first-class entity with start and end dates, an open/active/closed lifecycle, and Version-style sharing across a project tree. Redmine versions remain usable for planning alongside sprints. A board can be narrowed to one sprint, or follow whichever sprint is active.
...
* *Colours* — cards coloured by tracker, priority, status, project, assignee or spent time. Any colour can be picked in a colour picker that offers 18 palette shades as swatches, and a card's background tint stays light enough to read on whatever was chosen. Deliberately by what an issue is, never one issue at a time.
...
* No additional gems — Chart.js and the Coloris colour picker are vendored with the plugin
